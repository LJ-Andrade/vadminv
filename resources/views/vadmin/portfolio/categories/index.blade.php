...
{{-- HEAD--}}
@section('header')
	@section('header_title', 'Categorias del Portfolio') 
	@section('options')
		<div class="actions">
            <a href="{{ url('vadmin/port_categories/create') }}" class="btn btnSm buttonOther">Nueva Categoría</a>
            ...
            <div class="col-md-12 animated fadeIn main-list">
                ...
                <div id="Id{{ $item->id }}" class="Item-Row Select-Row-Trigger row item-row simple-list">
                    ...
                    <div class="List-Actions lists-actions Hidden">
                        <a href="{{ url('vadmin/port_categories/' . $item->id . '/edit') }}" class="ShowEditBtn btnSmall buttonOk" data-id="{{ $item->id }}">
...
@section('custom_js')
	...
	$(document).on('click', '.Delete', function(e){
		e.preventDefault();
		var id    = $(this).data('id');
		var route = "{{ url('vadmin/ajax_delete_port_category') }}/"+id+"";
	...
	function batch_delete_item(id) {
		...
		var route = "{{ url('vadmin/ajax_batch_delete_port_categories') }}/"+id+"";
